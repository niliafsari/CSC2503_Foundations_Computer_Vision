...
[pLeft polys MintLeft MintRight] = projectDino(f, dLeft, Rleft, sclZ);

%% Dino right image data
[pRight polys MintRight MextRight] = projectDino(f, dRight, Rright, sclZ);
% U=inv([MextRight; 0 0 0 1])
% U=[eye(3) zeros(3,1)]*(inv(MextRight'*MextRight)*MextRight')
...
x_vec=linspace(-150,150,2);
y_vec=linspace(-100,100,2);
nCol=length(x_vec)^2;
...
plot(gridpnt(:,1), gridpnt(:,2), '.','Color','b');
hold on
plot(imPts(1,:,1), imPts(2,:,1), '.r');
hold on
